...
var apiGoogleMobileAnalyticUrl = "../api/googleanalytic/custom/GetGoogleAnalytics/";
var pageSize = 10;
var PagerSize = 5;

dashboard.controller('AnalyticMobileLocationCtrl', function ($scope, analyticMobileLocationFactory, messageFactoryMobile) {
    $scope.pageNumber = 1;
    ...
    $scope.$watch('dateRangesModel', function (dateRange) {
        ...
        if (isFirstTime)
        {
            analyticMobileLocationFactory.GetGoogleMobileAnalytics('deviceCategory', $scope);
            isFirstTime = false;
        }
    ...
    $scope.Search = function () {
        ...
        if ($scope.startDate == '' || $scope.endDate == '') {
            messageFactoryMobile.error("Date field cann't be empty.");
        }
            ...
            if (monthDifference > -1)
            {
                ...
                messageFactoryAnalytic.error("Invalid date range. Please try again.");
            }
        }
...
dashboard.factory("analyticMobileLocationFactory", function ($http, messageFactoryMobile) {
    return {
       
        GetGoogleMobileAnalytics: function (dimenssion, $scope) {
            $http.get(apiGoogleMobileAnalyticUrl + '?isCompareExist=' + $scope.isCompare + '&dimensions=' + dimenssion + '&startDate=' + $scope.startDate + '&endDate=' + $scope.endDate + '&compareStartDate=' + $scope.compareStartDate + '&compareEndDate=' + $scope.compareEndDate)
            ...
            .error(function (msg) {
                $scope.GoogleAnalytics = false;
                messageFactoryMobile.error(msg);
            });
        }
    };
});
...
dashboard.factory('messageFactoryMobile', function () {
    return {
